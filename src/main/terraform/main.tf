...
locals {
  ...
  has_path   = var.artifact.path != null
  has_pwd    = var.credentials.password != null

  jar_hash = local.has_path ? filebase64sha256(var.artifact.path) : data.maven_artifact.publisher[0].output_base64sha256
  jar_path = local.has_path ? var.artifact.path : data.maven_artifact.publisher[0].output_path
}
...
data "aws_iam_policy_document" "publisher" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:ssm:*:*:parameter/${var.pstore_prefix}/*"
    ]
  }
}
...
resource "aws_ssm_parameter" "central_password" {
  name  = "/${var.pstore_prefix}/webhook-password"
  type  = "SecureString"
...
resource "aws_lambda_function" "publisher" {
  ...
  environment {
    variables = {
      ...
      WEBHOOK_USERNAME = aws_ssm_parameter.central_username.name
      WEBHOOK_PASSWORD = aws_ssm_parameter.central_password.name
      TZ               = var.timezone
    }
  }
